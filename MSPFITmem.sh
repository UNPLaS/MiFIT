...
WHERE=$(dirname "$0")
if [ $# -lt 5 ]; then
	echo Usage: $(basename "$0") '<filename.elf> <num of instructions> <num of injections> <reg with dir> <num of bytes> <Mod: 0-One Reg, 1-Random, 2-All reg> [Reg to inject (0 to 15)]' >&2
	exit 1
fi
...
awk '$1=="PC:" {printf $2"\n" }' $ramdir/Reg_gold.txt > $ramdir/RG0.txt
 ...
 dirmem="0x$(cat $ramdir/'RG'$reg_resul.txt)" 
 dirm="$(cat $ramdir/'RG'$reg_resul.txt):"
 arg3="md $dirmem $numBytes"
 #echo $arg3
 mspdebug $arg0 "$arg1" "$arg2" "$arg3"|sed -e 's/(//g'|sed -e 's/)//g' > $ramdir/MemG.dat
 tmp1=$(grep -n $dirm $ramdir/MemG.dat|cut -f1 -d":")
 let tmp=$tmp1-1
sed -i "1,$tmp"d $ramdir/MemG.dat
...
#    echo "Bit flip:" $nb
   ...
   arg4="step $inst_fini"
   arg5="md $dirmem $numBytes"
   mspdebug $arg0 "$arg1" "$arg2" "$arg3" "$arg4" "$arg5" > $ramdir/regs.txt 2> $ramdir/error.txt
 ...
 #       echo "hang1----"
  ...
  #          echo "hang2----"
            ...
            cp $ramdir/regs.txt $ramdir/Mem.dat
            tmp1=$(grep -n $dirm $ramdir/Mem.dat|cut -f1 -d":")
            let tmp=$tmp1-1
            sed -i "1,$tmp"d $ramdir/Mem.dat
            if diff $ramdir/Mem.dat $ramdir/MemG.dat >/dev/null; then
